...
data/clean/text_final/, xem legal_knowledge_graph/samples/). Mỗi câu có số
dòng KỲ VỌNG đã chạy thật trên Neo4j để xác nhận (không phải suy đoán) — nếu
sửa samples/ thì phải chạy lại và cập nhật các con số ở đây, `query_runner.py`
sẽ báo FAIL ngay nếu lệch.

`expected_rows = None` nghĩa là không có số cố định để assert (kết quả phụ
thuộc thuật toán mở rộng đồ thị/xếp hạng) — run_queries.py khi đó chỉ kiểm
tra có >=1 dòng và in ra để soi tay, không tính PASS/FAIL theo số dòng.
"""
...
class Query:
    ...
    kind: str  # lookup | multihop | fulltext | hybrid | sanity
    note: str
