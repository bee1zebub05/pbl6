...
def load_seed_entries(path: Path) -> list[dict]:
    """Đọc nguyên danh sách entry (dict) từ 1 file reference/*_seed.json —
    public, dùng lại được ở nơi khác ngoài validate.py (vd core/nlq/resolve.py
    cần tên hiển thị gốc, không chỉ slug)."""

    with open(path, encoding="utf-8") as f:
        raw = json.load(f)
    return raw.get("topics") or raw.get("organizations") or raw.get("target_groups") or []


def _load_seed_names(path: Path, key: str) -> set[str]:
    return {normalize.slug(item[key]) for item in load_seed_entries(path)}
...
def _seed_target_group_names() -> set[str]:
    return _load_seed_names(config.TARGET_GROUPS_SEED_PATH, "name")
...
def _cross_field_errors(data: dict) -> list[str]:
    ...
    target_group_seed = _seed_target_group_names()
    for group in doc.get("targetGroups") or []:
        if normalize.slug(group) not in target_group_seed:
            errors.append(
                f"document.targetGroups: '{group}' không khớp mục nào trong "
                "reference/target_groups_seed.json (dùng 'Khác' nếu không rõ nhóm)"
            )
...
def validate_dir(data_dir: Path) -> list[ValidationResult]:
    """Quét đệ quy (data thật nằm lồng theo <lĩnh vực>/<mã>.json). Bỏ qua mọi
    file nằm dưới một thư mục con bắt đầu bằng "_" — quy ước "gác lại, chưa
    xử lý" của kho data thật (vd _nghi_ngo/, _loai_bo/): chưa đọc tay xong
