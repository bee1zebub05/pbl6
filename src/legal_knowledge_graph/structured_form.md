...
| `targetGroups` | array of `string` | | Đối tượng áp dụng (thường ở Điều 1/2 "Phạm vi điều chỉnh và đối tượng áp dụng"), có thể nhiều hơn 1. **Phải khớp** một mục trong [`reference/target_groups_seed.json`](reference/target_groups_seed.json) (8 nhóm, xem [§7](#7-danh-mục-tham-chiếu)) — dùng `"Khác"` nếu không rõ nhóm nào. Kiểm tra ở `validate.py`. |
...
Ba file dùng để tra cứu giá trị hợp lệ khi điền `topics`/`targetGroups`/
`organization`, **không được tự bịa giá trị ngoài danh sách** cho
`topics`/`targetGroups`:
...
- [`reference/target_groups_seed.json`](reference/target_groups_seed.json)
  — 8 nhóm đối tượng áp dụng hợp lệ cho `document.targetGroups[]`: `Người
  học`; `Cán bộ, giảng viên, viên chức, người lao động`; `Đơn vị trực
  thuộc`; `Cơ sở giáo dục / trường đại học thành viên`; `Cơ quan, tổ chức,
  cá nhân có liên quan`; `Hội đồng và thành viên hội đồng`; `Doanh nghiệp,
  đối tác/tổ chức nước ngoài`; `Khác`. Danh sách này **tự thiết kế**, gom từ
  268 đoạn "Đối tượng áp dụng" thật trong `data/clean/text_final/` — khác
  `topics_seed.json` ở chỗ không có nguồn chuẩn sẵn nào khác trong repo để
  đối chiếu, nên có thể thiếu trường hợp hiếm; gặp đối tượng không khớp rõ
  nhóm nào thì dùng `"Khác"`, không ép vào nhóm sai.
...
thời**: nhiều `topics`, nhiều `targetGroups` (chính Điều 1 bên trong ghi rõ
"Áp dụng đối với các trường đại học thành viên, khoa, viện... trực thuộc" —
khớp cả 2 nhóm `Cơ sở giáo dục / trường đại học thành viên` và `Đơn vị trực
thuộc`), nhiều `citations` (toàn bộ đều `BASED_ON`), một `normativeContents`
có 3 Điều bên trong, **và** `articles` vỏ bọc cấp Document cùng lúc — trường
hợp phổ biến nhất trong kho dữ liệu thật.
...
```json
{
  ...
  "document": {
    ...
    "targetGroups": ["Cơ sở giáo dục / trường đại học thành viên", "Đơn vị trực thuộc"],
